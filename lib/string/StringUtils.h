...
public:
    ...
    static bool checkIsIpAddress(std::string ip) {
        int count = 0;
        for(auto s : ip) {
            if (s == '.') {
                count++;
    ...
    static unsigned int ipFromString(std::string ip) {
        uint32_t ret = 0;
        if(checkIsIpAddress(ip)) {
            std::vector<std::string> vect;
            str::Util::split(ip, ".", vect);
            uint8_t data1, data2, data3, data4;
            if(vect.size() == 4) {
                data1 = (uint8_t)convert<int>(vect.at(0), 0);
                data2 = (uint8_t)convert<int>(vect.at(1), 0);
                data3 = (uint8_t)convert<int>(vect.at(2), 0);
                data4 = (uint8_t)convert<int>(vect.at(3), 0);
                ret = data1 << 24 | data2 << 16 | data3 << 8 | data4;
            }
        }
        return ret;
    }
